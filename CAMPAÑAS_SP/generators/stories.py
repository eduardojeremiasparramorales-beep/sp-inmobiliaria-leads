...
def load_story_bg(project, img_key):
    ...
    if not path:
        return None
    img = load_and_crop(path, W, H, seed=f"{project.slug}:story-{img_key}")
...
def gen_bienvenido(project):
    ...
    draw_badge(draw, "LEONS GROUP", (cx - badge_w // 2, 680), badge_font, seed=f"{project.slug}:story-bienvenido-badge")
    text_with_shadow(draw, "Bienvenido a", (cx - 220, 740), Brand.font_cinzel(60), tracking=0.07)
    text_with_shadow(draw, "tu próximo", (cx - 200, 810), Brand.font_cinzel(60), tracking=0.07)
    text_with_shadow(draw, "proyecto", (cx - 180, 880), Brand.font_cinzel(60), tracking=0.07)
    if project.location:
...
def gen_area(project):
    ...
    text_with_shadow(draw, area_num or "98", (cx - 120, 750), Brand.font_cinzel(180), Brand.ORO, tracking=0.07)
    text_with_shadow(draw, f"{area_label.upper()} CUADRADOS" if area_label else "METROS CUADRADOS",
                     (cx - 250, 960), Brand.font_cinzel(32), tracking=0.07)
...
def gen_precio(project):
    ...
    text_with_shadow(draw, project.price, (cx - 300, 770), Brand.font_cinzel(80), Brand.ORO, tracking=0.07)
...
def gen_beneficios(project):
    ...
    draw = ImageDraw.Draw(img)
    add_logo(img, 100, (W - 100) // 2, 60, center=True)
    draw_badge(draw, "BENEFICIOS", (60, 650), seed=f"{project.slug}:story-beneficios-badge")
    text_with_shadow(draw, "Tu lote con", (60, 720), Brand.font_cinzel(50), tracking=0.07)
    text_with_shadow(draw, "todo incluido", (60, 780), Brand.font_cinzel(50), tracking=0.07)
    ...
    for feat in features:
        ...
        text_with_shadow(draw, feat, (90, y + 12), Brand.font_cinzel(22), Brand.ORO, tracking=0.07)
        y += 75
    return img
...
def gen_cta(project):
    ...
    text_with_shadow(draw, "Escríbenos", (280, 420), Brand.font_cinzel(56), tracking=0.07)
    text_with_shadow(draw, "por WhatsApp", (240, 490), Brand.font_cinzel(56), tracking=0.07)
    ...
    text_with_shadow(draw, "LEONS GROUP", (360, 1750), Brand.font_cinzel(18), Brand.ORO, tracking=0.07)
    return img
