...
def gen_facebook(project):
    ...
    if not path:
        return None
    img = load_and_crop(path, W, H, seed=f"{project.slug}:banner-facebook")
    img = add_gradient_left(img, 600, 230)
    draw = ImageDraw.Draw(img)
    add_logo(img, 60, 50, 30)
    draw_badge(draw, "LEONS GROUP", (50, 100), seed=f"{project.slug}:banner-facebook-badge")
    text_with_shadow(draw, "Lotes desde", (50, 150), Brand.font_cinzel(36), tracking=0.07)
    text_with_shadow(draw, project.price, (50, 200), Brand.font_cinzel(48), Brand.ORO, tracking=0.07)
    if project.location:
        text_with_shadow(draw, f"en {project.location.split(',')[0]}", (50, 260), Brand.font_cinzel(36), tracking=0.07)
...
def gen_instagram(project):
    ...
    text_with_shadow(draw, project.name, (100, 200), Brand.font_cinzel(44), tracking=0.07)
    if project.location:
        text_with_shadow(draw, project.location, (100, 260), Brand.font_inter(22), Brand.ORO)
    text_with_shadow(draw, project.price, (100, 330), Brand.font_cinzel(56), Brand.ORO, tracking=0.07)
